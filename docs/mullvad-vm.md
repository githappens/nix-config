...
Minimal NixOS VM running Mullvad Browser over a WireGuard full tunnel to Mullvad VPN. Ephemeral — reverts to clean snapshot on shutdown.
...
├── DNS via Mullvad (10.64.0.1, inside tunnel)
├── sway (minimal Wayland compositor)
└── Mullvad Browser (floating, letterboxed for fingerprint resistance)
```
...
5. Extract YubiKey SSH public key for VM access (stored in `secrets/yubikey-ssh.pub`, gitignored)
...
ssh user@<vm-ip>               # SSH in (uses YubiKey via gpg-agent)
```
